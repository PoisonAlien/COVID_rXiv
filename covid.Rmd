...
for(idx in seq_len(nrow(by_site))){
  ...
  plot(x = data$xax, y = data$total_cumsum, type = 'l', axes = FALSE, xlab = NA, ylab = NA, xlim = xlims, ylim = ylims, lwd = 1.2, col = "#2c3e50")
  text(x = by_site[idx, "xax"], y = by_site[idx, "total_cumsum"], labels = paste0("combined [", by_site[idx, "total_cumsum"], "]"), pos = 3, xpd = TRUE, col = "#2c3e50")
  ...
  title(main = paste0("bioRxiv = ", by_site[idx, "bioRxiv"], ", medRxiv = ", by_site[idx, "medRxiv"], ", total = ", by_site[idx, "total"]), adj = 0, line = 1, font.main = 3, col.main = "#34495e")
   ...
   if(max(data$total) > 10){
    b = boxplot(total ~ Month, data = data, axes = FALSE, xlab = NA, ylab = NA, outcex = 0.1, col = "#00b894")  
...
***Requires `ImageMagick` installation for gif creation
```{r, engine='bash'}
convert -loop 0 -delay 3 pngs_bysrc/*.png covid_rxiv.gif
```


<p align="left">
<img src="covid_rxiv.gif">
</p>
