...
meta_data = lapply(batches, function(batch){
...
#Save to file
data.table::fwrite(x = meta_data, file = "COVID_rxiv_2020-12-18.tsv", sep = "\t")

#Start from here if you have the file downloaded already
#meta_data = data.table::fread(input = "COVID_rxiv_2020-12-18.tsv")

meta_data = meta_data[order(rel_date, decreasing = FALSE)]
meta_data$ym = substr(meta_data$rel_date, 1, 7)

#Remove single pub from 2019:09
meta_data = meta_data[!ym %in% "2019-09"]

date_lvls = seq.Date(as.Date("2019/12/01"), as.Date("2021/01/01"), 1)
